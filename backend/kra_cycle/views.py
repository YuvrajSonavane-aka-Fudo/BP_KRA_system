...
def _clone_assignments(source_cycle, new_cycle, caller):
    ...
    source_ekcs = list(
        EmployeeKRACycle.objects.filter(
            kra_cycle=source_cycle
        ).select_related('employee__manager', 'employee__level', 'employee__role')
    )
    ...
    with transaction.atomic():
        for src_ekc in source_ekcs:
            ...
            invalid_cats   = [c.category_id for c in source_cats if c.category_id not in valid_cat_ids]
            ...
            valid_kra_rows    = [r for r in source_kra_rows if r.kra_level_id in valid_kra_level_ids]
            dropped_kra_ids   = [r.kra_level_id for r in source_kra_rows if r.kra_level_id not in valid_kra_level_ids]
    ...
    def post(self, request, cycle_id):
        ...
        _audit(
            ...
            new_data={
                ...
                'assignment_summary': assignment_result.get('summary') if assignment_result else None,
            },
        )
