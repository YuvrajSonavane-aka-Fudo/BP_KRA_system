...
export default function CycleCreateModal({ open= false, onClose, onSuccess }) {
  const [step, setStep] = useState(0); // 0 = timeline, 1 = stages
  ...
  useEffect(() => {
    if (open) {
      ...
      setFieldErrors({}); setTouched({}); setSubmitError('');
    }
  }, [open]);
  ...
  async function handleSubmit() {
    ...
    if (!step0Valid || !step1Valid) { setSubmitError('Please fix errors above.'); return; }
  ...
  return (
    ...
    >
      {/* ── Header ── */}
      <Box sx={{ background: gradient, px: 3, pt: 2.5, pb: 2, color: '#fff', position: 'relative' }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
          <Box>
            ...
            <Typography sx={{ fontSize: 12, opacity: 0.7, mt: 0.2 }}>
              {step === 0 ? 'Set name and cycle dates' : 'Set the date range for each stage'}
            </Typography>
          </Box>
      ...
      <Box sx={{ px: 3, py: 2.5, overflow: 'auto', flex: 1 }}>
        ...
        {step === 1 && (
          <Box sx={{ animation: 'slideIn 0.25s ease' }}>
            ...
            {submitError && <Alert severity="error" sx={{ mt: 1.5, fontSize: 12, borderRadius: 1.5 }} icon={<WarningAmberIcon fontSize="inherit" />}>{submitError}</Alert>}
          </Box>
        )}
      </Box>
      ...
      <Box sx={{ px: 3, py: 2, borderTop: '1px solid #f1f5f9', bgcolor: '#fafafa' }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          {step === 0 ? (
            <Button onClick={() => !submitting && onClose()} sx={{ color: '#94a3b8', fontWeight: 600, fontSize: 13 }}>
              Cancel
            </Button>
          ) : (
            <Button startIcon={<ArrowBackIcon />} onClick={goBack} disabled={submitting}
          ...
          {step === 0 ? (
            <Button
              ...
              sx={{
                ...
                fontWeight: 700, borderRadius: 2, px: 3, fontSize: 13,
                transition: 'all 0.2s',
