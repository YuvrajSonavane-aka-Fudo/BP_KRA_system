    ...
    <Dialog open={open} onClose={onGoBack} maxWidth="xs" fullWidth
      ...
      <Box sx={{ bgcolor: '#fffbeb', px: 2.5, pt: 2, pb: 1.5, borderBottom: '1px solid #fde68a' }}>
        <Stack direction="row" alignItems="center" spacing={1}>
          ...
          <Typography fontWeight={700} fontSize={14} color="#92400e">Before you go…</Typography>
        </Stack>
      </Box>
      <Box sx={{ px: 2.5, py: 2 }}>
        <Typography fontSize={13} color="#374151">
          Make sure your changes are saved before leaving. Go back to keep editing, or save everything now and close.
        </Typography>
      </Box>
      <Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ px: 2.5, pb: 2 }}>
        ...
        <Button onClick={onSaveClose} disabled={saving} variant="contained"
          ...
          sx={{ textTransform: 'none', fontWeight: 700, borderRadius: 1.5, px: 2.5, fontSize: 13, background: gradient, '&:hover': { opacity: 0.9 } }}>
    ...
    <Dialog open={open} onClose={onCancel} maxWidth="xs" fullWidth
      ...
      <Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ px: 2.5, pb: 2 }}>
        ...
        <Button onClick={onConfirm} disabled={loading} variant="contained"
          ...
          sx={{ textTransform: 'none', fontWeight: 700, borderRadius: 1.5, px: 2.5, fontSize: 13, bgcolor: confirmColor, '&:hover': { bgcolor: confirmColor, opacity: 0.88 } }}>
...
/* ════════════════════════════════════════
   Clone-to Inline Panel
   — cloning state is LOCAL (not from parent)
   — never calls onClose() after copy
   — shows inline success banner, panel stays open
════════════════════════════════════════ */
// ✅ CHANGE 1: Removed "cloning" from props — panel manages its own cloning state
function CloneTargetPanel({ employees, sourceEmployee, onClone, onClose }) {
  const [search, setSearch]           = useState('');
  const [selectedIds, setSelectedIds] = useState([]);
  const [copyDone, setCopyDone]       = useState(false);
  // ✅ CHANGE 2: cloning is now local state inside this component
  ...
  // ✅ CHANGE 3: handleClone manages local cloning state, never calls onClose()
    ...
    <Box sx={{ p: 1.75, bgcolor: '#f8fafc', borderRadius: 2, border: '1px solid #e2e8f0', mt: 0.75 }}>
      ...
      <Box sx={{ px: 1.5, py: 1, mb: 1.25, borderRadius: 1.5, bgcolor: '#eff6ff', border: '1px solid #bfdbfe' }}>
        <Typography fontSize={11} color="#1d4ed8" lineHeight={1.6}>
          Selected KRAs will be copied to the employees you choose. If someone already has a KRA, it won't be added again.
        </Typography>
      </Box>
      ...
      <Box sx={{
        ...
        border: '1px solid #e2e8f0', borderRadius: 1.5, mb: 1,
              ...
              return (
                <Box key={emp.employee_id} onClick={() => toggleId(emp.employee_id)}
                  ...
                  <Checkbox size="small" checked={sel}
                    onChange={() => toggleId(emp.employee_id)}
      ...
      {/* ✅ CHANGE 4: Inline success banner — panel stays open, no unmount */}
      {copyDone && (
        <Box sx={{
          ...
          <Typography fontSize={11} fontWeight={600} color="#166534">
            Done! KRAs copied successfully.
          </Typography>
        </Box>
      )}
      ...
      <Button variant="contained" fullWidth size="small"
        ...
        sx={{
          ...
          background: gradient, color: '#fff', borderRadius: 1.5, height: 32,
          '&:hover': { opacity: 0.9 },
...
export default function EmployeeKRAView({
  ...
  const [clonePanelFor, setClonePanelFor] = useState(null);
  // ✅ CHANGE 5: "cloning" state removed from here — it lives inside CloneTargetPanel now
  ...
  const toggleCategory = (catId, c) => {
    ...
    setCheckedKRAs(prev => {
      ...
      keys.forEach(k => c ? next.add(k) : next.delete(k));
      return next;
    });
  };
  ...
  const toggleKRA = (key, c) => {
    setCheckedKRAs(prev => {
      const next = new Set(prev); c ? next.add(key) : next.delete(key); return next;
    });
  };
  ...
  const handleClose = () => setWarnClose(true);
  ...
  // ✅ CHANGE 6: handleClone no longer sets cloning state — just calls the prop
    ...
    <>
      ...
      >
        ...
        <Box sx={{ background: gradient, px: 2.5, pt: 2, pb: 1.75, color: '#fff', flexShrink: 0 }}>
          <Stack direction="row" alignItems="center" spacing={1.5}>
            ...
            <Box sx={{ flexShrink: 0, minWidth: 0 }}>
              <Stack direction="row" alignItems="center" spacing={0.75}>
                ...
                <Chip
                  icon={<CheckCircleIcon sx={{ fontSize: '10px !important', color: '#86efac !important' }} />}
            ...
            <TextField
              size="small"
              placeholder={`Search across ${totalKRAs} KRAs…`}
              value={search}
              onChange={e => setSearch(e.target.value)}
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <SearchIcon sx={{ fontSize: 15, color: 'rgba(255,255,255,0.5)' }} />
                  </InputAdornment>
                ),
              }}
              sx={{
            ...
            <Tooltip title={checkedCount === 0 ? 'Select KRAs first to clone' : `Clone ${checkedCount} selected`}>
              <span>
                <Button size="small"
                  ...
                  onClick={() => setClonePanelFor(clonePanelFor === 'bulk' ? null : 'bulk')}
                  sx={{
          ...
          {/* Bulk clone panel — ✅ no cloning prop */}
          {clonePanelFor === 'bulk' && (
            <Box sx={{ mt: 1 }}>
              <CloneTargetPanel
        ...
        <Box sx={{
          ...
          <Stack direction="row" alignItems="center" spacing={1.5}>
            ...
            {wValid && (
              <Typography fontSize={11} color="#166534" fontWeight={500}>All weightages balanced ✓</Typography>
            )}
            {saveSuccess && (
              <Chip
                ...
                return (
                  <Paper key={group.category_id} elevation={0}
                    ...
                    <Box sx={{ bgcolor: tc.header, px: 1.5, py: 0.85 }}>
                      <Stack direction="row" alignItems="center" spacing={0.5}>
                        ...
                        <Stack direction="row" alignItems="baseline" spacing={0.75} flex={1}
                          ...
                          <Typography fontSize={13} fontWeight={700} color={tc.color}>
                            {group.category_name}
                            <Box component="sup"
                              sx={{ fontSize: 9, fontWeight: 800, ml: '2px', verticalAlign: 'super', lineHeight: 0 }}>
                              {group.rows.length}
                            </Box>
                          </Typography>
                          <Chip label={isOrg ? 'Org' : 'Project'} size="small"
                            sx={{ fontSize: 8.5, height: 16, fontWeight: 700, bgcolor: 'rgba(255,255,255,0.6)', color: tc.color }}
                          />
                        </Stack>
                        ...
                        <Tooltip title={`Copy all KRAs in "${group.category_name}"`}>
                          <IconButton size="small"
                            onClick={e => { e.stopPropagation(); setClonePanelFor(clonePanelFor === `cat-${group.category_id}` ? null : `cat-${group.category_id}`); }}
                            sx={{ p: 0.5, color: tc.color, '&:hover': { bgcolor: 'rgba(255,255,255,0.6)' } }}>
                            <ContentCopyIcon sx={{ fontSize: 15 }} />
                          </IconButton>
                        </Tooltip>

                        <Tooltip title={`Delete all KRAs in "${group.category_name}"`}>
                          <IconButton size="small"
                            onClick={e => {
                              e.stopPropagation();
                              setCheckedKRAs(new Set(group.rows.map(r => r.key)));
                              setWarnBulkDelete(true);
                            }}
                            sx={{ p: 0.5, color: '#ef4444', '&:hover': { bgcolor: 'rgba(239,68,68,0.1)' } }}>
                      ...
                      {/* Category-level clone panel — ✅ no cloning prop */}
                      {clonePanelFor === `cat-${group.category_id}` && (
                        <CloneTargetPanel
                          employees={employees.filter(e => e.assigned_to_cycle && e.employee_kra_cycle_id != null)}
                          sourceEmployee={employee}
                          onClone={handleClone}
                            ...
                            <Box key={row.key}>
                              <Stack direction="row" alignItems="center"
                                ...
                                <Checkbox size="small" checked={isChecked}
                                  ...
                                  sx={{ p: 0.4, mr: 0.75, flexShrink: 0, color: '#cbd5e1', '&.Mui-checked': { color: '#1E3A8A' } }}
                                />
                                ...
                                <Chip label={row.level_name} size="small"
                                  sx={{ fontSize: 9, height: 18, fontWeight: 600, bgcolor: '#f0f9ff', color: '#0369a1', mr: 0.75, flexShrink: 0 }} />

                                <Tooltip title="Copy this KRA to other employees">
                                  <IconButton size="small"
                                    onClick={() => setClonePanelFor(clonePanelFor === row.key ? null : row.key)}
                                    sx={{ p: 0.4, color: '#94a3b8', '&:hover': { color: '#1E3A8A', bgcolor: '#eff6ff' } }}>
                                    <ContentCopyIcon sx={{ fontSize: 14 }} />
                                  </IconButton>
                                </Tooltip>

                                <Tooltip title="Delete this KRA">
                                  <IconButton size="small"
                                    onClick={() => setWarnDeleteKRA(row.key)}
                                    sx={{ p: 0.4, color: '#94a3b8', '&:hover': { color: '#ef4444', bgcolor: '#fef2f2' } }}>
                                    <DeleteOutlineIcon sx={{ fontSize: 14 }} />
                                  </IconButton>
                                </Tooltip>
                              </Stack>

                              {/* Per-KRA clone panel — ✅ no cloning prop */}
                              {clonePanelFor === row.key && (
                                <Box sx={{ px: 2, pb: 1.25, bgcolor: '#fafafa', borderBottom: isLast ? 'none' : '1px solid #f1f5f9' }}>
                                  <CloneTargetPanel
                                    employees={employees.filter(e => e.assigned_to_cycle && e.employee_kra_cycle_id != null)}
                                    sourceEmployee={employee}
                                    onClone={handleClone}
            ...
            <Stack direction="row" alignItems="center" spacing={1}>
              ...
              <Button onClick={handleSave} disabled={saving || !dirty}
                ...
                sx={{
                  ...
                  borderRadius: 1.75, px: 2.5, height: 34,
                  background: gradient,
                  '&:hover': { opacity: 0.9 },
