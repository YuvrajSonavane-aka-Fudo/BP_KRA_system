...
import {
  ...
  Checkbox, Avatar, Snackbar, Select, MenuItem,
...
// ─── API imports ──────────────────────────────────────────────────────────────
// getKRALibrary comes from kraLibraryApi (calls /kra/library_kra) — single source of truth.
// assignmentsApi no longer exports its own getKRALibrary.
import { getKRALibrary } from '../../api/kraLibraryApi';
...
// is_standard: true  → ORG KRA    (defined by HR/Admin)   → green palette
// is_standard: false → PROJECT KRA (added by Lead/RM)     → blue palette
...
const EDIT_STAGES  = [1]; // Stage 1 = KRA Assignment By Lead

// is_standard: true → 'Org', false → 'Project'
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" gap={2}>
          <Stack direction="row" alignItems="center" gap={1.5} minWidth={0}>
            ...
            <Box minWidth={0}>
              <Stack direction="row" alignItems="center" gap={0.75} mb={0.25} flexWrap="wrap">
                <Chip
                  label={isReadOnly ? 'VIEW ONLY' : 'LIVE'}
                  size="small"
                  sx={{
                ...
                {cycle?.current_stage && (
                  <Chip
                    label={`Stage ${cycle.current_stage.id}: ${STAGE_LABELS[cycle.current_stage.id] ?? cycle.current_stage.name}`}
                    size="small"
                    sx={{ height: 17, fontSize: 9, fontWeight: 700, bgcolor: 'rgba(96,165,250,0.2)', color: '#bfdbfe' }}
                  />
                )}
              </Stack>
              <Typography fontWeight={800} fontSize="1rem" noWrap lineHeight={1.2}>
                {cycle?.name ?? 'No Active Cycle'}
              </Typography>
              <Typography fontSize={10.5} sx={{ opacity: 0.65, mt: 0.15 }}>
          ...
          <FormControl size="small" sx={{ minWidth: 190, flexShrink: 0 }}>
            <Select
              ...
              sx={{
                bgcolor: 'rgba(255,255,255,0.1)', color: '#fff', fontSize: 12, fontWeight: 600, borderRadius: 1.5,
            ...
            >
              {allCycles.active?.length > 0 && (
                <MenuItem disabled sx={{ fontSize: 10, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                  Active
                </MenuItem>
              )}
              {allCycles.active?.map(c => (
                <MenuItem key={c.id} value={c.id} sx={{ fontSize: 12, fontWeight: 600 }}>{c.name}</MenuItem>
              ))}
              {allCycles.closed?.length > 0 && (
                <MenuItem disabled sx={{ fontSize: 10, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', mt: 0.5 }}>
                  Closed (Read Only)
                </MenuItem>
              )}
              {allCycles.closed?.map(c => (
                <MenuItem key={c.id} value={c.id} sx={{ fontSize: 12, color: '#94a3b8' }}>{c.name}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Stack>
...
function KRAPanel({ kras, categories, selectedKraLevelIds, onToggleKRA, isReadOnly, employeeDuplicateMap }) {
  ...
  const [typeFilter, setTypeFilter] = useState('all'); // all | org | project
  const [levelFilter, setLevelFilter] = useState('');

  // ── FIX: collapsed defaults to {} so all categories start EXPANDED.
  // A category is collapsed only if collapsed[cid] === true (user clicked to close).
  const [collapsed, setCollapsed] = useState({});

  // Build unique levels from library
  ...
  // Group & filter
  // ── FIX: group.isStd is derived per-category from the KRA's own is_standard.
  // Each KRA row carries its own is_standard — use the first KRA in the group
  // as the representative (all KRAs in a category share the same is_standard).
  const grouped = useMemo(() => {
    ...
    list.forEach(kra => {
      ...
      if (!map[cid]) {
        map[cid] = {
          cid,
          name: kra.category_name || categories.find(c => c.id === cid)?.name || `Category ${cid}`,
          // ── FIX: isStd comes from the KRA's is_standard, not from the category.
          // is_standard on a KRA: true = Org, false = Project.
          isStd: kra.is_standard,
          kras: [],
        };
      }
  ...
  // Toggle collapse: true = collapsed (hidden), undefined/false = expanded (visible)
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 0 }}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1.5}>
        <Stack direction="row" alignItems="center" gap={1}>
          <Typography fontWeight={800} fontSize={13} color="#0f172a" letterSpacing="-0.01em">KRA Library</Typography>
          <Chip
            label={`${kras.length}`}
            size="small"
        ...
        <Stack direction="row" gap={1} alignItems="center">
          <Tooltip title="Organisation KRAs — defined by HR/Admin (is_standard: true)">
            <Chip
              icon={<Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: ORG_COLOR.icon, ml: '6px !important' }} />}
              label="Org"
              size="small"
              sx={{ height: 20, fontSize: 9.5, fontWeight: 700, bgcolor: ORG_COLOR.chip, color: ORG_COLOR.text, cursor: 'default' }}
            />
          </Tooltip>
          <Tooltip title="Project KRAs — added by Team Lead (is_standard: false)">
            <Chip
              icon={<Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: PROJ_COLOR.icon, ml: '6px !important' }} />}
              label="Project"
              size="small"
          ...
          {selectedKraLevelIds.length > 0 && (
            <Chip
              label={`${selectedKraLevelIds.length} selected`}
              size="small"
      ...
      <TextField
        size="small"
        placeholder="Search KRAs by title or description…"
        value={search}
        onChange={e => setSearch(e.target.value)}
        InputProps={{
          startAdornment: (
            <InputAdornment position="start"><SearchIcon sx={{ fontSize: 15, color: '#94a3b8' }} /></InputAdornment>
          ),
          endAdornment: search ? (
      ...
      <Stack direction="row" gap={1} mb={1.5} alignItems="center">
        {/* Org / Project / All toggle */}
        <Stack direction="row" sx={{ border: '1px solid #e2e8f0', borderRadius: 1.5, overflow: 'hidden', flexShrink: 0, height: 30 }}>
          {[
            { v: 'all',     l: 'All' },
            { v: 'org',     l: `Org (${orgCount})` },
            { v: 'project', l: `Proj (${projectCount})` },
          ].map(t => (
            <Box
              key={t.v}
              onClick={() => setTypeFilter(t.v)}
              sx={{
                px: 1.25, display: 'flex', alignItems: 'center', fontSize: 10.5, fontWeight: 700,
        ...
        <FormControl size="small" sx={{ flex: 1 }}>
          ...
          >
            ...
            {allLevels.map(l => (
              <MenuItem key={l.id} value={String(l.id)} sx={{ fontSize: 11 }}>{l.name}</MenuItem>
            ))}
          </Select>
        </FormControl>
      </Stack>
      ...
      <Box sx={{
        ...
        <Checkbox
          size="small"
          disabled={isReadOnly || allVisibleIds.length === 0}
          indeterminate={someSelected && !allSelected}
          checked={allSelected}
          onChange={e => onToggleKRA(allVisibleIds, e.target.checked ? 'select_all' : 'deselect_all')}
          sx={{ p: 0.5 }}
        />
      ...
      <Box sx={{
        ...
        {grouped.length === 0 ? (
          ...
          const catIds   = group.kras.flatMap(k => k.levels?.map(l => l.kra_level_id) ?? []);
          const catAll   = catIds.length > 0 && catIds.every(id => selectedKraLevelIds.includes(id));
          const catSome  = catIds.some(id => selectedKraLevelIds.includes(id));

          // ── FIX: isOpen is true when collapsed[cid] is NOT true (default = open)
          const isOpen   = collapsed[group.cid] !== true;
          const tc       = typeColor(group.isStd); // isStd: true=Org(green), false=Project(blue)
          const selCount = catIds.filter(id => selectedKraLevelIds.includes(id)).length;
            ...
            <Box key={group.cid} sx={{ mb: 0.75 }}>
              ...
              <Box sx={{
                ...
                <Checkbox
                  ...
                  checked={catAll && catIds.length > 0}
                  onChange={e => {
                    e.stopPropagation();
                    onToggleKRA(catIds, e.target.checked ? 'select_all' : 'deselect_all');
                  }}
                  onClick={e => e.stopPropagation()}
                  sx={{ p: 0.5, mr: 0.5, flexShrink: 0, color: tc.text, '&.Mui-checked': { color: tc.text } }}
                />
                <Stack
                  direction="row" alignItems="center" gap={0.75} flex={1} minWidth={0}
                  onClick={() => toggleCollapse(group.cid)}
                  sx={{ cursor: 'pointer' }}
                >
                  <Typography fontSize={12} fontWeight={800} color={tc.text} noWrap sx={{ letterSpacing: '-0.01em' }}>
                    {group.name}
                  </Typography>
                  {/* ── FIX: typeLabel uses group.isStd — true='Org', false='Project' */}
                  <Chip
                    label={typeLabel(group.isStd)}
                    size="small"
                    sx={{ height: 15, fontSize: 8, fontWeight: 800, bgcolor: tc.chip, color: tc.text, borderRadius: 0.75, letterSpacing: '0.04em' }}
                  />
                  <Chip
                    label={`${group.kras.length} KRA${group.kras.length !== 1 ? 's' : ''}`}
                    size="small"
              ...
              {/* KRA rows — visible by default (isOpen = true initially) */}
              <Collapse in={isOpen}>
                <Box sx={{ pl: 1.5 }}>
                  {group.kras
                    .slice()
                    ...
                    .map(kra =>
                      (kra.levels ?? []).map(level => {
                        const isSelected   = selectedKraLevelIds.includes(level.kra_level_id);
                        // Each KRA row uses its own is_standard for the badge color
                        const ktc          = typeColor(kra.is_standard);
                        const dupCount     = employeeDuplicateMap?.[level.kra_level_id] ?? 0;
                        const hasDuplicate = dupCount > 0;

                        return (
                          <Box
                            key={level.kra_level_id}
                            onClick={() => {
                              if (!isReadOnly) onToggleKRA([level.kra_level_id], isSelected ? 'deselect' : 'select');
                            }}
                            sx={{
                              display: 'flex', alignItems: 'center', px: 1.25, py: 0.85, borderRadius: 1.5, mb: 0.3,
                              ...
                              '&:hover': {
                                bgcolor: isReadOnly ? 'transparent' : isSelected ? '#eff6ff' : '#f8fafc',
                          ...
                          >
                            ...
                            <Box flex={1} minWidth={0}>
                              <Stack direction="row" alignItems="center" gap={0.5} flexWrap="wrap">
                                ...
                                {hasDuplicate && (
                                  <Tooltip title={`${dupCount} selected employee${dupCount > 1 ? 's' : ''} already have this KRA assigned`}>
                                    <Chip
                                      label={`${dupCount} already assigned`}
                                      size="small"
                            ...
                            {/* ── FIX: each KRA row badge uses its own kra.is_standard */}
                            <Chip
                              label={typeLabel(kra.is_standard)}
                              size="small"
                              sx={{
                                height: 15, fontSize: 8, ml: 0.75, flexShrink: 0, fontWeight: 700,
...
function EmployeePanel({ employees, selectedEmployeeIds, onToggleEmployee, isReadOnly, canEdit,
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1.5}>
        <Stack direction="row" alignItems="center" gap={1}>
          <Typography fontWeight={800} fontSize={13} color="#0f172a" letterSpacing="-0.01em">Target Employees</Typography>
        ...
        <Stack direction="row" gap={0.5}>
          {[
            { l: 'All',        c: employees.length, v: 0 },
            { l: 'Assigned',   c: assigned,         v: 1 },
            { l: 'Unassigned', c: unassigned,        v: 2 },
          ].map(t => (
            <Chip
              key={t.v}
              label={`${t.l} ${t.c}`}
              ...
              onClick={() => setTab(t.v)}
              sx={{
                height: 22, fontSize: 10, fontWeight: 700, cursor: 'pointer', borderRadius: 1.5,
                bgcolor: tab === t.v ? '#1E3A8A' : '#f1f5f9',
      ...
      <Stack direction="row" gap={1} mb={1}>
        ...
        <Tooltip title="Filters">
          <Badge
            badgeContent={hasFilters ? (!!deptFilter + !!vertFilter + !!levelFilter) : 0}
            color="primary"
          ...
          >
            <IconButton
              size="small"
              sx={{
                border: `1px solid ${hasFilters ? '#93c5fd' : '#e2e8f0'}`, borderRadius: 1.5,
      ...
      <Stack direction="row" gap={1} mb={1.5}>
        {[
          ...
          <FormControl key={i} size="small" sx={{ flex: 1 }}>
            <Select
              ...
              displayEmpty
              sx={{
                fontSize: 10.5, height: 30, borderRadius: 1.5,
                bgcolor: f.val ? '#eff6ff' : '#f8fafc',
                color: f.val ? '#1d4ed8' : undefined,
        ...
        {hasFilters && (
          <Button
            size="small"
            onClick={() => { setDept(''); setVert(''); setLevel(''); }}
            sx={{ fontSize: 10, color: '#64748b', minWidth: 0, px: 1, height: 30, flexShrink: 0 }}
          >
            Clear
          </Button>
        )}
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', px: 1, pb: 0.75, mb: 0.5, borderBottom: '1px solid #f1f5f9' }}>
        <Checkbox
          ...
          onChange={e => onToggleEmployee(filtered.map(e => e.employee_id), e.target.checked ? 'select_all' : 'deselect_all')}
          sx={{ p: 0.5 }}
        />
        ...
        {filtered.length === 0 ? (
          ...
          return (
            ...
            >
              ...
              {/* Name + email */}
              <Stack
                direction="row" alignItems="center" gap={1.25} flex={1} minWidth={0} ml={0.5}
              ...
              <Box sx={{ width: 130, mr: 1, flexShrink: 0 }}>
                ...
                <Typography fontSize={10} color="#94a3b8" noWrap>
                  {[emp.level, emp.department].filter(Boolean).join(' · ')}
                </Typography>
              </Box>
                ...
                {isAssigned && (
                  <>
                    ...
                    <Tooltip title="Copy KRAs to others">
                      <IconButton
                        size="small" onClick={() => onClone(emp)}
                    ...
                    {canEdit && (
                      <Tooltip title="Remove from cycle">
                        <IconButton
                          size="small" onClick={() => onDelete(emp)}
...
function SelectionBar({ selectedKraIds, selectedEmpIds, employees, kraLibrary, assignedEmployees,
  ...
  const fullyDuplicateEmps = employees.filter(e => {
    if (!selectedEmpIds.includes(e.employee_id)) return false;
    if (!e.assigned_to_cycle) return false;
  ...
  const partialDuplicateEmps = employees.filter(e => {
    if (!selectedEmpIds.includes(e.employee_id)) return false;
    if (!e.assigned_to_cycle) return false;
    if (fullyDuplicateEmps.includes(e)) return false;
    const cached = assignedEmployees[e.employee_kra_cycle_id];
    if (!cached) return false;
  ...
  return (
    <Paper elevation={0} sx={{
      position: 'sticky', bottom: 0, zIndex: 10, borderRadius: 2.5,
      border: '1px solid #bfdbfe', bgcolor: '#fff',
      boxShadow: '0 -8px 32px -8px rgba(30,58,138,0.15)',
      overflow: 'hidden',
    }}>
          ...
          <Stack direction="row" alignItems="center" gap={1}>
            ...
            <Typography fontSize={11} color="#92400e" fontWeight={600}>
              {fullyDuplicateEmps.length === selectedEmpIds.length
                ? `All ${fullyDuplicateEmps.length} selected employee${fullyDuplicateEmps.length > 1 ? 's' : ''} already have these KRAs assigned — re-assigning will update their existing records.`
                : `${fullyDuplicateEmps.length} employee${fullyDuplicateEmps.length > 1 ? 's' : ''} already ha${fullyDuplicateEmps.length > 1 ? 've' : 's'} all selected KRAs: ${fullyDuplicateEmps.slice(0, 2).map(e => e.full_name.split(' ')[0]).join(', ')}${fullyDuplicateEmps.length > 2 ? ` +${fullyDuplicateEmps.length - 2}` : ''}`
              }
            </Typography>
          </Stack>
        </Box>
        ...
        <Box sx={{ px: 2.5, py: 0.75, bgcolor: '#f0f9ff', borderBottom: '1px solid #bae6fd' }}>
          <Stack direction="row" alignItems="center" gap={1}>
            ...
            <Typography fontSize={11} color="#0c4a6e" fontWeight={500}>
              {partialDuplicateEmps.length} employee{partialDuplicateEmps.length > 1 ? 's' : ''} already {partialDuplicateEmps.length > 1 ? 'have' : 'has'} some of these KRAs — they will receive the additional ones.
            </Typography>
          </Stack>
        </Box>
      )}
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" flexWrap="wrap" gap={1.5}>
            ...
            <Stack direction="row" sx={{ '& > *:not(:first-of-type)': { ml: '-8px' } }}>
              {avatarEmps.map(e => (
                <Tooltip key={e.employee_id} title={e.full_name}>
                  <Avatar sx={{ width: 28, height: 28, fontSize: 10, fontWeight: 800, background: G, border: '2px solid #fff', borderRadius: 1.25, flexShrink: 0 }}>
                    {initials(e.full_name)}
                  </Avatar>
                </Tooltip>
              ))}
              {selectedEmpIds.length > 5 && (
                <Avatar sx={{ width: 28, height: 28, fontSize: 9, fontWeight: 700, bgcolor: '#e2e8f0', color: '#64748b', border: '2px solid #fff', borderRadius: 1.25 }}>
                  +{selectedEmpIds.length - 5}
                </Avatar>
              )}
            </Stack>
            <Box>
              <Typography fontSize={12} fontWeight={800} color="#0f172a" letterSpacing="-0.01em">Selection Summary</Typography>
              <Stack direction="row" alignItems="center" gap={0.75} flexWrap="wrap">
                ...
                {readyCount !== selectedEmpIds.length && readyCount > 0 && (
                  <Chip
                    label={`${readyCount} new assignments`}
                    size="small"
          ...
          <Stack direction="row" alignItems="center" gap={0.75} flexWrap="wrap">
            {selectedKraIds.length > 0 && (
              <Button
                size="small" onClick={onClearKRAs}
                startIcon={<ClearIcon sx={{ fontSize: 12 }} />}
                sx={{ fontSize: 11, fontWeight: 600, color: '#64748b', border: '1px solid #e2e8f0', borderRadius: 1.5, height: 30, px: 1.5, '&:hover': { border: '1px solid #cbd5e1', bgcolor: '#f8fafc' } }}
              >
                KRAs
              </Button>
            )}
            {selectedEmpIds.length > 0 && (
              <Button
                size="small" onClick={onClearEmps}
                startIcon={<ClearIcon sx={{ fontSize: 12 }} />}
                sx={{ fontSize: 11, fontWeight: 600, color: '#64748b', border: '1px solid #e2e8f0', borderRadius: 1.5, height: 30, px: 1.5, '&:hover': { border: '1px solid #cbd5e1', bgcolor: '#f8fafc' } }}
              >
                Employees
            ...
            <Button
              size="small" onClick={onClearAll}
              sx={{ fontSize: 11, fontWeight: 600, color: '#94a3b8', height: 30, px: 1, '&:hover': { color: '#64748b' } }}
            >
              Clear All
            </Button>
            <Button
              ...
              sx={{
                fontSize: 12, fontWeight: 800, background: G, color: '#fff', borderRadius: 1.5,
                height: 36, px: 2.5, letterSpacing: '-0.01em',
                boxShadow: '0 2px 12px rgba(30,58,138,0.35)',
                '&:hover': { background: 'linear-gradient(135deg, #1e40af 0%, #1d4ed8 100%)', boxShadow: '0 4px 20px rgba(30,58,138,0.45)' },
...
function ToastStack({ toasts }) {
  return (
    <Box sx={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 2000, display: 'flex', flexDirection: 'column', gap: 1, alignItems: 'center' }}>
      {toasts.map(t => (
        <Fade key={t.id} in>
          <Alert severity={t.severity} sx={{ borderRadius: 2, fontWeight: 600, fontSize: 12.5, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', minWidth: 300, maxWidth: 480 }}>
            {t.msg}
          </Alert>
        </Fade>
      ))}
...
export default function BulkAssignmentPage() {
  ...
  // assignedKRAMap: employee_kra_cycle_id → kra_level_ids[]
  ...
  // Prevents React 18 StrictMode from double-invoking the bootstrap effect,
  // which would fire all 4 API calls twice (categories, cycles, library_kra, employees).
  ...
  // Build employeeDuplicateMap: kra_level_id → count of selected employees that already have it
  ...
  // ── Bootstrap: getKRALibrary called via loadKRAAssignmentPageData
  // which imports it from kraLibraryApi.js → /kra/library_kra
  //
  // hasMounted ref prevents React 18 StrictMode from double-firing this effect
  // in development (mount → unmount → remount), which duplicates all 4 API calls.
  useEffect(() => {
  ...
  const fetchEmployees = useCallback(async (cycleId) => {
    try {
      setRefetching(true);
      const res  = await getEmployees(cycleId);
      const emps = res.data.employees ?? [];
      setEmployees(emps);
      rebuildAssignedMap(emps);
    } catch {
      showToast('Could not refresh the employee list.', 'error');
    } finally {
      setRefetching(false);
    }
  }, [rebuildAssignedMap]);
  ...
  const handleAssignClick = () => {
    ...
    if (selectedEmployeeIds.length === 0) { showToast('Select at least one employee to assign KRAs to.', 'warning'); return; }

    const allDuplicate = employees
      .filter(e => selectedEmployeeIds.includes(e.employee_id) && e.assigned_to_cycle && e.employee_kra_cycle_id)
      .every(e => {
        const cached = assignedKRAMap[e.employee_kra_cycle_id] ?? [];
        return selectedKraLevelIds.every(id => cached.includes(id));
      });

    if (
      allDuplicate &&
      selectedEmployeeIds.every(id => employees.find(e => e.employee_id === id)?.assigned_to_cycle)
    ) {
      showToast('All selected employees already have these exact KRAs. Use the edit (✏) button to modify.', 'info');
      return;
    }

    const selectedKras = kraLibrary.filter(k =>
      k.levels?.some(l => selectedKraLevelIds.includes(l.kra_level_id))
    );
    ...
    setWeightageModal({
      ...
      prefill: { categories: prefillCats, kra_level_ids: selectedKraLevelIds },
    });
  };
  ...
  const handleConfirmAssign = async ({ categories: cats, kra_level_ids, is_date_based }) => {
    ...
    try {
      ...
      await fetchEmployees(activeCycle.id);
      setSelectedKraLevelIds([]);
      setSelectedEmployeeIds([]);

      if (failed.length === 0) {
        showToast(
          `${enrolled.length > 0 ? `${enrolled.length} employee${enrolled.length !== 1 ? 's' : ''} assigned successfully.` : ''}${skipped.length > 0 ? ` ${skipped.length} already assigned (updated).` : ''}`,
          'success'
        );
      } else if (enrolled.length > 0) {
        showToast(`${enrolled.length} assigned, ${failed.length} failed. Please try again for the failed employees.`, 'warning');
      } else {
        showToast('Assignment failed. Please check your selections and try again.', 'error');
      }
    } catch (e) {
      showToast(e?.response?.data?.message || 'Assignment failed. Please try again.', 'error');
    } finally {
      setAssigning(false);
    }
  };
  ...
  const handleEditEmployee = (emp) => {
    ...
    setWeightageModal({
      open: true, mode: 'edit', employee: emp,
      prefill: cached
        ? { categories: cached.categories, kra_level_ids: cached.kra_level_ids }
        : { categories: [], kra_level_ids: [] },
    });
  };
  ...
  const handleConfirmEdit = async ({ categories: cats, kra_level_ids, is_date_based }) => {
    ...
    try {
      ...
      saveAssignmentToCache({
        employee_kra_cycle_id: employee.employee_kra_cycle_id,
        cycle_id: activeCycle.id,
        categories: cats,
        kra_level_ids,
      });
  ...
  const handleDeleteEmployee = async (emp) => {
    try {
      ...
      await fetchEmployees(activeCycle.id);
      showToast(`${emp.full_name} removed from this cycle.`, 'success');
    } catch (e) {
      showToast(e?.response?.data?.message || 'Remove failed. Please try again.', 'error');
    }
  };

  const handleBulkDelete = async () => {
    const toDelete = employees.filter(
    ...
    try {
      ...
      await fetchEmployees(activeCycle.id);
      setSelectedEmployeeIds([]);
      showToast(`${toDelete.length} employee${toDelete.length !== 1 ? 's' : ''} removed from this cycle.`, 'success');
    } catch {
      showToast('Bulk remove failed. Please refresh and try again.', 'error');
    }
  };
  ...
  const handleCloneTo = async (targetIds) => {
    try {
      ...
      await fetchEmployees(activeCycle.id);
      showToast(`KRAs from ${viewEmployee.full_name} copied to ${targetIds.length} employee${targetIds.length !== 1 ? 's' : ''}.`, 'success');
      setViewEmployee(null);
      setCloneDefault(false);
    } catch {
      showToast('Copy failed. Make sure all target employees are enrolled in this cycle.', 'error');
    }
  };

  const selectedAssigned = employees.filter(
    ...
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', bgcolor: '#f8fafc', overflow: 'hidden' }}>
      ...
      <Box sx={{ px: 3, pt: 2.5, pb: 1.5, flexShrink: 0 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Box>
            <Typography fontWeight={900} color="#0f172a" fontSize="1.1rem" letterSpacing="-0.02em" lineHeight={1.2}>
              KRA Assignment
            </Typography>
            <Typography fontSize={12} color="#94a3b8" mt={0.25}>
              Assign performance targets to employees for the active cycle
            </Typography>
          </Box>
          <Stack direction="row" gap={1} alignItems="center">
            {selectedAssigned.length > 0 && canEdit && (
              <Button
                ...
                sx={{
                  ...
                  border: '1px solid #fecaca', borderRadius: 1.5, height: 32, px: 1.5,
            ...
            <Tooltip title="Refresh employee list">
              <IconButton
                size="small"
                onClick={() => activeCycle && fetchEmployees(activeCycle.id)}
                disabled={refetching}
                sx={{
                  border: '1px solid #e2e8f0', borderRadius: 1.5, width: 32, height: 32, color: '#64748b',
              ...
              >
                <RefreshIcon
                  fontSize="small"
                  sx={{
                    animation: refetching ? 'spin 1s linear infinite' : 'none',
      ...
      <Box sx={{ flex: 1, overflow: 'auto', px: 3, pb: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        ...
        {isReadOnly && activeCycle && (
          <Alert severity="info" icon={<LockIcon fontSize="small" />} sx={{ borderRadius: 2, fontSize: 12, py: 0.5 }}>
            <strong>View Only.</strong> This cycle is {activeCycle.status?.toLowerCase()}. KRA assignment is only available for active cycles.
          </Alert>
        )}
        {!isReadOnly && !canEdit && (
          <Alert severity="warning" sx={{ borderRadius: 2, fontSize: 12, py: 0.5 }}>
            <strong>Limited access.</strong> Cycle is past Stage 1. You can view assignments but not edit or remove them.
          </Alert>
        )}
        ...
        {activeCycle ? (
          ...
          <Paper elevation={0} sx={{ borderRadius: 2.5, border: '1px solid #e2e8f0', p: 6, textAlign: 'center', bgcolor: '#fff' }}>
            ...
            <Typography fontSize={15} fontWeight={800} color="#374151" letterSpacing="-0.02em">No Active Cycle</Typography>
            <Typography fontSize={13} color="#94a3b8" mt={0.5}>
              Activate a KRA cycle to start assigning KRAs to employees.
            </Typography>
          </Paper>
        )}
        ...
        {!isReadOnly && (selectedKraLevelIds.length > 0 || selectedEmployeeIds.length > 0) && (
          <SelectionBar
            ...
            assignedEmployees={assignedKRAMap}
            onClearKRAs={() => setSelectedKraLevelIds([])}
            onClearEmps={() => setSelectedEmployeeIds([])}
      ...
      {/* Modals */}
      <ManageWeightageModal
        open={weightageModal.open}
        mode={weightageModal.mode}
        employee={weightageModal.employee}
        prefill={weightageModal.prefill}
        kraLibrary={kraLibrary}
        categories={categories}
        selectedEmployeeIds={selectedEmployeeIds}
        employees={employees}
